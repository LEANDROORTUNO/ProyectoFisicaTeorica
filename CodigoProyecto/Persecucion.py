...
class ProyectoPersecucion(Scene):
    def construct(self):
        ...
        ejes = NumberLine(x_range=[0, 500, 100], length=10, include_numbers=True).shift(DOWN)
        #imagenes de autos
        try:
        ...
        carro_a.move_to(ejes.n2p(vehiculo1.getPosInicial())).shift(UP*0.5)
        carro_b.move_to(ejes.n2p(vehiculo2.getPosInicial())).shift(UP*0.5)
        ...
        carro_a.add_updater(lambda m: m.move_to(ejes.n2p(vehiculo1.getPosicionFinal(reloj.get_value()))).shift(UP*0.2)) 
        carro_b.add_updater(lambda m: m.move_to(ejes.n2p(vehiculo2.getPosicionFinal(reloj.get_value()))).shift(UP*0.2)) 
